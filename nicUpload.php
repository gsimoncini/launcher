...
define('NICUPLOAD_PATH', './files/doc_content'); // Set the path (relative or absolute) to
// the directory to save image files

define('BASE_URL', 'http://localhost/bc-drp_politicas/');

define('NICUPLOAD_URI', 'http://localhost/bc-drp_politicas/files/doc_content');   // Set the URL (relative or absolute) to
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Upload is complete
    if (empty($id) || !is_numeric($id)) {
        nicupload_error('ID de Carga Inválido');
    }
    ...
    if (!$file) {
        nicupload_error('El archivo debe tener menos de ' . bytes_to_readable($max_upload_size));
    }
    ...
    if (!move_uploaded_file($image, $path)) {
        nicupload_error('Server error, Error al mover el archivo.');
    }
...
function nicupload_output($status, $showLoadingMsg = false) {
    ...
    echo '<html><body>
        <div id="uploadingMessage" style="text-align: center; font-size: 14px;">
            <img src="'.BASE_URL.'/img/load.gif" style="float: right; margin-right: 40px;" />
